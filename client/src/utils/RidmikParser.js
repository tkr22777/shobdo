...
import BanglaUnicode from './BanglaUnicode.js';

export default function RidmikParser() {

    let unicode = new BanglaUnicode();
    ...
    this.toBangla = function (engWord) {
        ...
        let secondCarry = 0;
        let thirdCarry = 0;
        let tempNoCarry = false;
        let jukta = false;
        let prevJukta = false;

        for (let p in engWord) {
            let now = engWord[p];
            ...
            if (isConsonant(now) && isConsonant(carry) && !tempNoCarry){
                ...
                // check if dual
                let dual = unicode.getDual(now, carry);
                ...
                if (dual !== undefined){
                    ...
                    let firstOrAfterVowelOrJukta = isVowel(secondCarry) || secondCarry === 0 || prevJukta;
    ...
    function dualSitsUnder(thirdCarry, secondCarry, carry, now){
        ...
        let djkt = unicode.getDjkt(carry, now);
        if (djkt !== undefined)
            if (isCharInString(secondCarry, djkt))
                return true;

        let djktt = unicode.getDjktt(carry, now);
    ...
    function notJukta(thirdCarry, secondCarry, carry, now) {
        ...
        let foo = unicode.getDualJkt(secondCarry, carry);
